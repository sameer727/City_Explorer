  ...
  const onSubmit = async (event) => {
    ...
    try {
      ...
      if (!requestError?.response) {
        ...
        setError(apiMessage || 'Login failed. Please check your details.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <section className="mx-auto grid min-h-[70vh] max-w-6xl items-center gap-10 px-4 py-10 sm:px-6 lg:grid-cols-2 lg:px-8">
        <div>
          ...
          <h1 className="mt-4 text-4xl font-bold text-brand-ink dark:text-white">
            Sign in to unlock premium travel data
          </h1>
          <p className="mt-4 text-slate-500">
            Attractions, reviews, and favorites are protected routes in your backend.
          </p>
        </div>
        ...
        <form onSubmit={onSubmit} className="rounded-[24px] bg-white p-7 shadow-soft dark:bg-slate-800">
          ...
          <p className="mt-4 text-center text-sm text-slate-500 dark:text-slate-400">
            Don't have an account? <Link to="/register" className="text-brand-accent hover:underline">Create one</Link>
          </p>
        </form>
      </section>
    </PageTransition>
